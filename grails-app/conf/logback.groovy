...
appender("ROLLING-ERROR", RollingFileAppender) {
    ...
    rollingPolicy(TimeBasedRollingPolicy) {
        fileNamePattern = "${LOG_ARCHIVE}/errorFile.%d{yyyy-MM-dd-HH}.zip"
        //property defines a file name pattern for archived log files
        maxHistory = 30
...
appender("ROLLING-INFO", RollingFileAppender) {
    ...
    rollingPolicy(TimeBasedRollingPolicy) {
        fileNamePattern = "${LOG_ARCHIVE}/infoFile.%d{yyyy-MM-dd-HH}.zip"
        maxHistory = 30
